...
const Page = () => {
  ...
  useEffect(() => {
    ...
    const normalizedItems = Array.isArray(buyNowItem)
      ? buyNowItem
      : [buyNowItem];
  ...
  }, [isCheckingToken, isLogin, router]);
  ...
  useEffect(() => {}, [removeItemFromOrders]);
  ...
  const handleCheckout = async (e) => {
    ...
    const totalAmountNumber = itemsToCheckout.reduce(
      (acc, item) =>
        acc + item.quantity * (item?.price || item?.productId?.price || 0),
      0,
    );
    ...
    const normalizedProducts = itemsToCheckout.map((prd) => {
      ...
      return {
        ...
        productBrand:
          prd.brand || prd.productBrand || prd.productId?.brand || "",
        quantity: prd.quantity,
        price: actualPrice,
        image: prd.image || prd.images?.[0] || prd.productId?.images?.[0] || "",
  ...
  return (
    <div className="flex w-full min-h-screen justify-center bg-slate-100">
      <div className="w-[1020px] flex flex-col gap-2 my-3 mx-auto ">
        ...
        <div className="p-5   bg-white  shadow-slate-400 shadow-lg">
          <div className="flex justify-between items-center">
            ...
            <Button variant="outlined" onClick={toggleAddressSelectButton}>
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-start mb-2">
                          <div
                            ...
                            onClick={() =>
                              router.replace(
                                `/product/${item?.name ? item?._id : item?.productId}`,
                              )
                            }
                          >
      ...
      {showCodConfirm && (
        <div className="fixed inset-0 z-[1999] flex justify-center items-center bg-black bg-opacity-50">
